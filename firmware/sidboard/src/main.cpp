 ...
 *
 * Architettura:
 *  - Decodifica esterna indirizzi ($D800-$DFFF) via 74HC138 (Y7 -> ROM_SEL).
 *  - Core 1: MonitorTask — bus monitor ad alta velocità, nessuna variabile condivisa.
 *  - Core 0: loop() + esp_timer (audio cSIDLight) + task sistema.
 *  - Comunicazione: FreeRTOS Queue (solo per eventi PBI enable/disable).
 *
 * Mappa $D1xx:
 *  - $D100-$D11F  Registri SID emulati da cSIDLight (abilitati da $D1FF bit 0)
 *  - $D120-$D1FE  Registri interni ESP32
 *  - $D1FF        Bit 0 = 1: abilita device / Bit 0 = 0: disabilita + reset SID
 *
 * Pin Mapping:
 *  - Data Bus  D0-D7 : GPIO 18, 19, 21, 22, 23, 3, 26, 27
 *  - Addr Bus  A0-A5 : GPIO 32, 33, 34, 35, 36, 39
 *  - Addr Bus  A6-A10: GPIO 16, 17, 14, 12, 13
 *  - PHI2            : GPIO 2    (input)
 *  - R/W             : GPIO 15   (input, High=Read)
 *  - D1XX            : GPIO 5    (input, Active Low)
 *  - ROM_SEL         : GPIO 4    (input, Active Low — 74HC138)
 *  - EXTSEL          : GPIO 0    (output, Active Low)
 *  - Audio DAC       : GPIO 25   (output — cSIDLight, 22050 Hz)
 *  - Serial TX       : GPIO 1    (output, TX-Only debug)
 */
...
// PIN_DEV_SEL (GPIO 3) removed to free pin for Data Bus D5
...
void IRAM_ATTR MonitorTask(void *pvParameters)
{
    ...
    for (;;)
    {
        ...
        bool is_esp32_range = is_d1xx && (offset >= 0x20 && offset < 0xFF);  // $D120-$D1FE internal
        ...
        else
        {
            ...
            if (is_d1xx)
            {
                ...
                else if (local_pbi_enabled && off8 < 0x20) // $D100-$D11F SID registers
                {
        ...
        GPIO.out_w1ts = (1UL << PIN_EXTSEL); // Rilascio Segnali
    }
}
...
void setup()
{
    ...
    // Initialize cSIDLight (DAC on GPIO 25, esp_timer at 22050 Hz) before starting MonitorTask
    sidLight = new cSIDLight();
    Serial.println("[sidboard] cSIDLight initialized on $D100-$D11F, DAC GPIO25");

    // Start Monitor on Core 1 (after sidLight is ready)
    xTaskCreatePinnedToCore(MonitorTask, "PBI", 4096, NULL, 10, NULL, 1);
}

void loop()
{
    // Scan SID registers for changes and update audio synthesis parameters
    sidLight->process();
    ...
    if (xQueueReceive(pbiEventQueue, &pbi_state, 10 / portTICK_PERIOD_MS))
    {
        ...
        if (!pbi_state)
            sidLight->reset();  // Silence all voices when device is disabled
    }
}
